...
/**
 * Что останется в ленте после чистки перед запуском действия из заметки. При
 * включённом «беречь журнал правок» — одни карточки (в них живёт кнопка
 * «Отменить правку»), иначе панель начинается с пустого места: правку запускают
 * из заметки, и отвечать на неё смотрят сюда, а не читают прошлые.
 *
 * Тем же ответом решается, чистить ли вообще: осталось столько же, сколько
 * было, — уведомление «вернуть разговор» пообещало бы возврат ничего.
 */
export function afterFresh(history: HistoryItem[], keepLog: boolean): HistoryItem[] {
  return keepLog ? dropTalk(history) : [];
}

/**
 * Лента, возвращённая нажатием по уведомлению. Голой подменой снимком обойтись
 * нельзя: уведомление живёт восемь секунд — ровно столько модель и правит
 * текст, — и всё, что легло в ленту после снимка, подмена унесла бы. Дороже
 * всего карточка идущей правки: текст в заметке уже заменён, а кнопки
 * «Отменить правку» к нему не осталось бы.
 */
export function mergeRestored(kept: HistoryItem[], current: HistoryItem[]): HistoryItem[] {
  const known = new Set(kept);
  return [...kept, ...current.filter((item) => !known.has(item))];
}
