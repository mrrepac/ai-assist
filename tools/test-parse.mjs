...
const { formatText } = await load("src/format.ts", "format");
...
check(
  ...
  ["spelling", "format", "clarify", "expand", "shorten", "evaluate", "transcript"],
);
// Промпта у местного действия нет, и в API оно не ходит — этим и ценно.
check("форматирование работает без модели", mergeSettings(null).actions.find((a) => a.id === "format").local, true);
check("у форматирования нет промпта", mergeSettings(null).actions.find((a) => a.id === "format").prompt, "");
...
check(
  ...
  mergeSettings({ actions: [{ id: "custom-1", name: "Моё", prompt: "п", mode: "chat", icon: "x" }] })
    .actions.map((a) => a.id),
  ["spelling", "format", "clarify", "expand", "shorten", "evaluate", "transcript", "custom-1"],
);
...
// Перевод ушёл из слотов, на его место встало форматирование.
check(
...
// ——— форматирование ———
// Чистка идёт без модели, значит спросить её «а точно так?» не у кого: правило
// должно срабатывать одинаково всегда. Половина проверок про то, чего трогать
// нельзя, — там пробел значит ровно то, что написано.
const fmt = (text, trimEdges = true) => formatText(text, { trimEdges }).text;

check("двойные пробелы схлопываются", fmt("два  пробела"), "два пробела");
check("одиночный пробел в конце убирается", fmt("строка \nвторая"), "строка\nвторая");
check("три пробела в конце — мусор", fmt("строка   \nвторая"), "строка\nвторая");
// Ровно два пробела — жёсткий перенос markdown, в стихах их ставят нарочно.
check("жёсткий перенос остаётся", fmt("Златая цепь на дубе том:  \nИ днём и ночью"), "Златая цепь на дубе том:  \nИ днём и ночью");
check("жёсткий перенос переживает схлопывание", fmt("два  слова  \nдальше"), "два слова  \nдальше");
check("жёсткий перенос переживает правку знака", fmt("Его острейшей ,  \nдальше"), "Его острейшей,  \nдальше");
check("три пустые строки становятся одной", fmt("абзац\n\n\n\n\nвторой"), "абзац\n\nвторой");
// Пустая строка отделяет строфу — одну оставляем всегда.
check("одна пустая строка остаётся", fmt("строфа\n\nстрофа"), "строфа\n\nстрофа");
check("строка из пробелов считается пустой", fmt("раз\n   \n\nдва"), "раз\n\nдва");
check("отступ списка не трогается", fmt("- пункт\n    - вложенный  пункт"), "- пункт\n    - вложенный пункт");
check("пробел перед запятой убирается", fmt("да , конечно"), "да, конечно");
check("пробел перед точкой в конце строки убирается", fmt("вышел тихо ."), "вышел тихо.");
check("смайлик не портится", fmt("ну ладно :)"), "ну ладно :)");
// Дальше — места из хранилища, на которых широкое правило делало хуже.
check("двойная запятая не склеивается", fmt("да ведь, , Малые Мытищи"), "да ведь, , Малые Мытищи");
check("уехавший не на ту сторону пробел не трогаем", fmt("и ,чёрт знает"), "и ,чёрт знает");
check("разреженное многоточие в реплике остаётся", fmt(". . . odours savours"), ". . . odours savours");
check("пробел перед многоточием авторский", fmt("до Лунной горы! …А любовник"), "до Лунной горы! …А любовник");
check("десятичная запятая не трогается", fmt("ровно 3 ,5 метра"), "ровно 3 ,5 метра");
check("неразрывный пробел становится обычным", fmt("в\u00A01941\u00A0году"), "в 1941 году");
check("знак нулевой ширины выкидывается", fmt("сло\u200Bво"), "слово");
check("мягкий перенос из PDF выкидывается", fmt("пере\u00ADнос"), "перенос");
check(
  "блок кода остаётся как есть",
  fmt("текст\n```\nlet a  =  1;\n\n\n\nlet b = 2;\n```"),
  "текст\n```\nlet a  =  1;\n\n\n\nlet b = 2;\n```",
);
// Имя файла в ссылке — не текст: правка внутри скобок рвёт связь между заметками.
check("имя файла в ссылке не трогаем", fmt("[[Моя  заметка]] и  текст"), "[[Моя  заметка]] и текст");
check("код в строке не трогаем", fmt("вот `a  b` тут"), "вот `a  b` тут");
check("колонки таблицы держатся", fmt("| а   | б   |\n| --- | --- |"), "| а   | б   |\n| --- | --- |");
check("шапка YAML не трогается", fmt("---\ntags:  [а]\n---\n\n\n\nтекст"), "---\ntags:  [а]\n---\n\nтекст");
check("пустые строки в конце заметки убираются", fmt("текст\n\n\n"), "текст");
check("править нечего — текст тот же", fmt("чистый текст"), "чистый текст");
// Выделение сделали руками: пробел по краям — стык с соседним текстом, и без
// него замена склеит слова.
check("края своего выделения не трогаются", fmt(" слово  и   ещё\n", false), " слово и ещё\n");
check(
  "счёт убранного",
  (() => {
    const s = formatText("раз  два\n\n\n\nтри", { trimEdges: true }).stats;
    return [s.blanks, s.spaces];
  })(),
  [2, 1],
);
check(
  "невидимое считается отдельно",
  formatText("в\u00A01941\u00A0году", { trimEdges: true }).stats.invisible,
  2,
);
